...
private String generateBoardKey(CXBoard B) {
    StringBuilder sb = new StringBuilder();

    // Aggiungi le informazioni sulla dimensione del tabellone
    sb.append(getRows()).append("_").append(getColumns()).append("_");

    // Aggiungi gli stati delle celle del tabellone
    for (int row = 0; row < getRows(); row++) {
        for (int col = 0; col < getColumns(); col++) {
            sb.append(B.cellState(row, col).ordinal());
        }
    }
...
//evaluateMove ora, implementa un alpha-beta pruning
	private int evaluateMove(CXBoard B, int column, boolean maxPlayer, int depth, int alpha, int beta) throws TimeoutException {
		...
		String key1 = generateBoardKey(B);
		cache.put(key1, bestScore);
