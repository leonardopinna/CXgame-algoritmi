   ...
   //metodo principale di selezione di una colonna, da getbestmove partono gli altri metodi
	...
	public int iterativeDeepening(CXBoard B, int depth) throws TimeoutException {
		...
		try {
			...
			// Per ogni profonfità da 1 a depth, calcolo un punteggio
			for (int d = 1; d <= depth; d++) {
				
				checktime();
				// Per ogni mossa, calcolo un punteggio
				...
				// Crea una lista di coppie (Q[i], scores[i])
				List<Pair<Integer, Integer>> pairList = new ArrayList<>();
				for (int i = 0; i < Q.length; i++) {
					pairList.add(new Pair<>(Q[i], scores[i]));
				}
	
				// Ordina la lista in base al punteggio in ordine decrescente
				pairList.sort((a, b) -> b.getValue().compareTo(a.getValue()));
	
				// Aggiorna gli array Q e scores con gli elementi ordinati
				...
				// Aggiorna la migliore mossa trovata in questa iterazione
